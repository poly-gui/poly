...
interface INativeLayerService {
  ...
  showAttachedDialog(title: string, message: string, windowTag: string): void;
...
class NativeLayerServiceServer extends RpcServer {
  constructor(
    ...
    this.on("show_attached_dialog", (reader, offset, msgId) => {
      let ptr = offset;
      const titleByteLength = reader.readInt32(ptr);
      ptr += 4;
      const title = reader.readString(ptr, titleByteLength);
      ptr += titleByteLength;
      const messageByteLength = reader.readInt32(ptr);
      ptr += 4;
      const message = reader.readString(ptr, messageByteLength);
      ptr += messageByteLength;
      const windowTagByteLength = reader.readInt32(ptr);
      ptr += 4;
      const windowTag = reader.readString(ptr, windowTagByteLength);
      ptr += windowTagByteLength;

      this.impl.showAttachedDialog(title, message, windowTag);
      const writer = new NanoBufWriter(6, false);
      writer.appendUint8(RpcMessageType.RESPONSE);
      writer.appendUint32(msgId);
      writer.appendUint8(0);
    ...
    this.on("show_dialog", (reader, offset, msgId) => {
      let ptr = offset;
      const titleByteLength = reader.readInt32(ptr);
      ptr += 4;
      const title = reader.readString(ptr, titleByteLength);
      ...
      this.impl.showDialog(title, message);
      const writer = new NanoBufWriter(6, false);
      writer.appendUint8(RpcMessageType.RESPONSE);
      writer.appendUint32(msgId);
      writer.appendUint8(0);
...
class NativeLayerServiceClient extends RpcClient {
  ...
  async showAttachedDialog(
    ...
    writer.appendUint8(RpcMessageType.REQUEST);
    writer.appendUint32(msgId);
    writer.appendStringAndSize("show_attached_dialog");
    const titleByteLength = writer.appendStringAndSize(title);
    const messageByteLength = writer.appendStringAndSize(message);
    const windowTagByteLength = writer.appendStringAndSize(windowTag);
  ...
  async showDialog(title: string, message: string): Promise<void> {
    const writer = new NanoBufWriter(9 + 11, false);
    const msgId = this.newMessageId();
    writer.appendUint8(RpcMessageType.REQUEST);
    writer.appendUint32(msgId);
    writer.appendStringAndSize("show_dialog");
    const titleByteLength = writer.appendStringAndSize(title);
    const messageByteLength = writer.appendStringAndSize(message);
